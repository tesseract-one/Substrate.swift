...
extension Ss58AddressFormat: Codable {
    public init(from decoder: Decoder) throws {
        ...
        guard let format = Ss58AddressFormat(rawValue: uint) else {
            throw DecodingError.dataCorruptedError(
                in: container,
                debugDescription: "Bad Ss58AddressFormat value \(uint)"
            )
        }
...
public struct Ss58AddressCodec {
    public func decode(string: String) throws -> (Data, Ss58AddressFormat) {
        let data = try base58_decode(string)
        guard data.count >= 2 else { throw Ss58Error.badLength }
        ...
        case 64..<128:
            ...
            prefixLen = 2
        default: throw Ss58Error.unknownVersion
        }
        
        let cLength = try checksumLength(for: data.count, prefix: prefixLen)
        guard let format = Ss58AddressFormat(rawValue: ident) else {
            throw Ss58Error.formatNotAllowed
        }
        ...
        guard checksum == data.suffix(cLength) else {
            throw Ss58Error.invalidChecksum
        }
        return (body.suffix(from: prefixLen), format)
    }
    
    public func encode(data: Data, format: Ss58AddressFormat) -> String {
    ...
    public func checksumLength(for dataLength: Int, prefix: Int) throws -> Int {
        ...
        if prefix == 1 {
            ...
            case 34, 35: return 2
            default: throw Ss58Error.badLength
            }
        } else if prefix == 2 {
            switch bodyLength {
            case 34, 35: return 2
            ...
            throw Ss58Error.badLength
        }
    }
    
    public static let prefix = Data("SS58PRE".utf8)
    public static let instance = Ss58AddressCodec()
    public static let defaultEncodeChecksumLength = 2
}

public enum Ss58Error: Error {
    case badLength
    case unknownVersion
